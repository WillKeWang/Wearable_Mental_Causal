"""
Modular wearable data preprocessing utilities for TemPredict dataset.

This module provides configurable preprocessing of Oura wearable data with survey responses,
supporting various time windows, baseline adjustments, and statistical aggregations.
"""
...
class ProcessingConfig:
    ...
    stat_functions: List[str] = field(default_factory=lambda: ["mean", "std", "rms", "skew"])
...
class WearablePreprocessor:
    ...
    def _convert_dateutc(self, df: pd.DataFrame, src="DateUTC.x", new="date", tz="UTC") -> pd.DataFrame:
        """Convert DateUTC column to proper datetime. Might need to test for additional formats."""
    ...
    def _rms_ignore_nan(self, a2d: np.ndarray) -> np.ndarray:
        """Vectorized RMS calculation, NaN-robust."""
        if a2d.size == 0:
            return np.full(a2d.shape[1] if a2d.ndim == 2 else 1, np.nan)
        with np.errstate(all="ignore"):
            return np.sqrt(np.nanmean(a2d**2, axis=0))
    
    def compute_statistics(self, window_data: pd.DataFrame) -> Dict[str, float]:
        """Compute statistical aggregations for window data."""
        ...
        with np.errstate(all="ignore"):
            stats = {
                ...
                "rms": self._rms_ignore_nan(arr),
                "skew": skew(arr, axis=0, nan_policy="omit", bias=False)
            }
        
        # Format results
        result = {}
        for j, col in enumerate(self.config.metric_cols):
            for stat_name in self.config.stat_functions:
                if stat_name in stats:
                    result[f"{col}_{stat_name}"] = stats[stat_name][j]
...
# Unit Testing
class TestWearablePreprocessor(unittest.TestCase):
    """Unit tests for WearablePreprocessor."""
    ...
    def test_window_direction(self):
        """Test window direction detection."""
        backward_config = ProcessingConfig(window_start_offset=-30, window_end_offset=0)
        self.assertEqual(backward_config.window_direction, "backward")
        
        forward_config = ProcessingConfig(window_start_offset=0, window_end_offset=30)
        self.assertEqual(forward_config.window_direction, "forward")
        
        mixed_config = ProcessingConfig(window_start_offset=-10, window_end_offset=10)
        self.assertEqual(mixed_config.window_direction, "mixed")
    
    def test_filename_generation(self):
        """Test filename generation."""
        filename = self.config.generate_filename("main")
        self.assertIn("7d_before_to_0d_before", filename)
        self.assertIn("baseline_adj", filename)
        self.assertIn("n5", filename)
    
    def test_rms_calculation(self):
        """Test RMS calculation."""
        preprocessor = WearablePreprocessor(self.config)
        
        # Test with valid data
        data = np.array([[1, 2], [3, 4], [5, 6]])
        result = preprocessor._rms_ignore_nan(data)
        expected = np.sqrt(np.mean(data**2, axis=0))
        np.testing.assert_array_almost_equal(result, expected)
        
        # Test with NaN data
        data_nan = np.array([[1, np.nan], [np.nan, 4], [5, 6]])
        result_nan = preprocessor._rms_ignore_nan(data_nan)
        self.assertFalse(np.isnan(result_nan).all())
    ...
    def test_statistics_computation(self):
        ...
        stats = preprocessor.compute_statistics(test_data)
        
        # Check that all expected statistics are computed
        expected_keys = ['hr_average_mean', 'hr_average_std', 'hr_average_rms', 'hr_average_skew',
                        'rmssd_mean', 'rmssd_std', 'rmssd_rms', 'rmssd_skew']
